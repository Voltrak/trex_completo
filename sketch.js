...
var Ptero, ImagenPtero, GrupoPtero;
var score = 0;
var PLAY = 1;
var END = 0;
var gameState = PLAY;
...
//y end es cuando ya termina el juego por ejemplo cuando mueres y se reinicia tambien conocido como "gameover"
 ...
 trex.y = trex.y + 7
  ...
  if(obstacleGroup.isTouching(trex) || GrupoPtero.isTouching(trex)){
    gameState = END
    sonidomuerte.play();
...
function spawnClouds(){
  if(frameCount % 60 === 0){
    nube = createSprite((Math.round(random(600,900))),100,40,10);
...
function MostrarPtrodactilos(){
  if(frameCount % 755 === 0){
    Ptero = createSprite(600, (Math.round(random(90,160))), 40, 10)
